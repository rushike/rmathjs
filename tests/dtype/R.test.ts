...
describe("test real (R) dtype : ", ()=>{
  it("test parse function --> ", ()=>{
    var a = [
      ...
      expected = [
            ...
            {n : 3, b : 10, e : 0, p : 1},
            {n : 6, b : 10, e : -3, p : 1},
            {n : -65698660128975165187657638219395n, b : 10, e : 0, p : 32}
          ].map(real)
    ;
  ...
  test("test sub method -> ", ()=>{
    var a = [11, "100.212",0, 89.898, 78, 1, "-0.65698660128975165187657638219395"].map(real),
    b = [3, "0.3","121.90", "0", "899", -0.4, "-0.65698660128975165187657638219383"].map(real)
    ;
    var expected = [8, "99.912","-121.9", 89.898, -821, 1.4, "-0.00000000000000000000000000000012"].map(real);
